...
public class EmojiSearch(ClipboardService clipboard, string emojiCachePath, EmojiDataLoader dataLoader, EmojiUsageStore usageStore, ILogger<EmojiSearch> logger, UserSettings settings) : IInstantSearchSource {
    ...
    private IReadOnlyList<(EmojiEntry Entry, EmojiSection Section)> GetDefaultEmojis() {
        ...
        // Most-used next, filling up to EmojiMaxPinnedTotal total pinned cells, excluding favorites.
        // Most-used also appear in their Default position below.
        var mostUsedMax = AppDefaults.EmojiMaxPinnedTotal - usageStore.Favorites.Count;
