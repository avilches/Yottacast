...
El algoritmo de scoring de cada fuente está documentado en `docs/search-scoring.md`. Cubre:

- **NameMatcher** — scoring para `ApplicationSearch` y `EmojiSearch` (CamelHump, iniciales, abreviatura, substring).
- **UserDocumentSearch** — scoring client-side sobre el nombre del fichero (single-token, multi-token, wildcard).
- **Scores entre fuentes** — cómo se ordenan los resultados de distintas fuentes en el merge global.
...
**Windows — file search**: ejecuta un script PowerShell codificado en Base64 que abre una conexión OLE DB al `SystemIndex` (`Provider=Search.CollatorDSO`) y lanza una query SQL con `CONTAINS(System.FileName, 'token*')` por cada token y `System.ItemPathDisplay LIKE 'folder%'` para el scope. Los caracteres `'`, `"` y `*` se eliminan de la query para evitar inyección SQL. Usa `StandardCommandRunner` (redirige stdout, no PTY).

**Linux — file search**: usa `plocate` si existe en `/usr/bin/plocate`, sino `locate`. Solo pasa el primer token al binario con pattern `*token*`; los tokens adicionales y el filtro de carpetas se aplican client-side en el callback. Usa `StandardCommandRunner`.

**`StandardCommandRunner`**: lee stdout línea a línea con `ReadLineAsync(ct)`. Al cancelar o al salir del bucle (porque `onLine` devolvió `false`), llama `proc.Kill(entireProcessTree: true)` para asegurar que ningún subproceso quede huérfano.
